...
- **One upstream endpoint is currently broken server-side** (the CLI builds the request correctly): `oa create-prebuiltapps` (`POST /oa/prebuilt-apps/configurations/query`) returns HTTP 404 (route absent). This is an Addigy API issue, not a CLI defect.
- **Billing endpoints are not wrapped.** The `/o/{org}/billing/*` API surface (account, cards, invoices, contact) is deliberately excluded from this CLI.
