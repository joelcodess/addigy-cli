...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("ADDIGY_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("ADDIGY_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "addigy-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.addigy-cli/feedback.jsonl.
When ` + "`ADDIGY_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`ADDIGY_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  addigy-cli feedback list
  addigy-cli feedback list --limit 5
  addigy-cli feedback list --json`,
