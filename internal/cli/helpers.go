...
// globalFlagNames are the root persistent flags; they must never be forwarded
// as request query parameters by deleteQueryParams.
var globalFlagNames = map[string]bool{
	"agent": true, "compact": true, "config": true, "csv": true, "data-source": true,
	"deliver": true, "dry-run": true, "human-friendly": true, "idempotent": true,
	"ignore-missing": true, "json": true, "no-cache": true, "no-color": true,
	"no-input": true, "plain": true, "profile": true, "quiet": true, "rate-limit": true,
	"select": true, "timeout": true, "yes": true,
}

// deleteQueryParams collects the command's set (changed) local flags as request
// query parameters, converting kebab-case flag names to the snake_case the API
// expects (e.g. --policy-id -> policy_id). Addigy v2 delete endpoints identify
// the target via query params, so without this the id/key is never transmitted.
func deleteQueryParams(cmd *cobra.Command) map[string]string {
	params := map[string]string{}
	cmd.Flags().Visit(func(fl *pflag.Flag) {
		if globalFlagNames[fl.Name] {
			return
		}
		params[strings.ReplaceAll(fl.Name, "-", "_")] = fl.Value.String()
	})
	return params
}
...
func classifyAPIError(err error, flags *rootFlags) error {
	...
	case strings.Contains(msg, "HTTP 400") && cliutil.LooksLikeAuthError(msg):
		return authErr(fmt.Errorf("%w\nhint: the API rejected the request — this usually means auth is missing or invalid."+
			"\n      Set your API key: export ADDIGY_DOCUMENTATION_API_KEY=<your-key>"+
			"\n      Get a key at: https://app.addigy.com/integrations"+
			"\n      Run 'addigy-cli doctor' to check auth status."+
			"\n      Response: "+cliutil.SanitizeErrorBody(msg), err))
	case strings.Contains(msg, "HTTP 401"):
		return authErr(fmt.Errorf("%w\nhint: check your API key."+
			" Set it with: export ADDIGY_DOCUMENTATION_API_KEY=<your-key>"+
			"\n      Get a key at: https://app.addigy.com/integrations"+
			"\n      Run 'addigy-cli doctor' to check auth status.", err))
