...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  addigy-cli workflow archive

  # Full re-archive (ignore previous sync state)
  addigy-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"configuration", "device-script-assignments", "facts", "feature-betas", "mdm", "mdm-configurations-definitions", "mdm-configurations-policy-profiles", "mdm-configurations-profiles", "monitoring", "oa", "oa-benchmarks-pre-built", "oa-compliance-rules-pre-built", "oa-integrations-azure-ca-tenants-metadata", "static-fields", "static-fields-value", "system-updates", "system-updates-available"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  addigy-cli workflow status

  # Show status as JSON
  addigy-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("addigy-cli")
			}
