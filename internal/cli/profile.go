...
// one named context that a scheduled agent reuses run after run with the same
// configuration but different inputs.
type Profile struct {
...
func profileStorePath() (string, error) {
	...
	dir := filepath.Join(home, ".addigy-cli")
...
present (other than --profile and --config).`,
		Example: `  addigy-cli profile save my-defaults --json --compact
  addigy-cli profile save tonight-defaults --region US`,
...
func newProfileUseCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: `  addigy-cli profile use my-defaults
  addigy-cli profile use tonight-defaults --json`,
		Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
...
func newProfileListCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: `  addigy-cli profile list
...
func newProfileShowCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: `  addigy-cli profile show my-defaults
  addigy-cli profile show tonight-defaults --json`,
		Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			p, err := GetProfile(args[0])
			if err != nil {
...
func newProfileDeleteCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: `  addigy-cli profile delete my-defaults --yes
  addigy-cli profile delete old-profile --yes --json`,
		Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			name := args[0]
			s, err := loadProfileStore()
