...
large datasets as it has no memory pressure.`,
  ...
  addigy-cli export <resource> --format jsonl --output data.jsonl

  # Export with limit
  addigy-cli export <resource> --format jsonl --limit 1000

  # Pipe to another tool
  addigy-cli export <resource> --format jsonl | jq '.id'`,
		Args: cobra.MinimumNArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			validResources := map[string]bool{
				"device-script-assignments":  true,
				"feature-betas":              true,
				"managed-app-configurations": true,
				"static-fields":              true,
			}
