...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  addigy-cli doctor
  addigy-cli doctor --json
  addigy-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export ADDIGY_DOCUMENTATION_API_KEY=<your-key>"
					report["auth_key_url"] = "https://app.addigy.com/integrations"
				} else {
					authConfigured = true
			...
			if os.Getenv("ADDIGY_DOCUMENTATION_API_KEY") != "" {
				authEnvSet = append(authEnvSet, "ADDIGY_DOCUMENTATION_API_KEY")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "ADDIGY_DOCUMENTATION_API_KEY")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					// against a lightweight read endpoint (the API key's own
					// permission list — no PII, cheap). A 401/403 means the key
					// is bad; a real health check must surface that rather than
					// green-light a broken config as "configured".
					...
					if authHeader == "" {
						...
						_, verifyErr := c.Get("/configuration/permissions", nil)
						var verifyAPIErr *client.APIError
						switch {
						case verifyErr == nil:
							report["credentials"] = "valid (authenticated read succeeded)"
						case errors.As(verifyErr, &verifyAPIErr) && (verifyAPIErr.StatusCode == 401 || verifyAPIErr.StatusCode == 403):
							report["credentials"] = fmt.Sprintf("invalid (HTTP %d — key rejected by the API)", verifyAPIErr.StatusCode)
							credentialsInvalid = true
						case errors.As(verifyErr, &verifyAPIErr):
							report["credentials"] = fmt.Sprintf("present (could not verify: HTTP %d)", verifyAPIErr.StatusCode)
						default:
							report["credentials"] = "skipped (verify call failed)"
						}
					}
				}
			...
			if flags.asJSON {
				...
				if credentialsInvalid {
					return authErr(fmt.Errorf("doctor: API credentials are invalid (authenticated probe rejected with 401/403)"))
				}
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'addigy-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'addigy-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'addigy-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'addigy-cli sync' to refresh."
	}
	return report
}
