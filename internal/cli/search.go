...
In local mode: searches locally synced data only.`,
  ...
  addigy-cli search "error timeout"

  # Force local search only
  addigy-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  addigy-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  addigy-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: POST /system-events/search
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, _, getErr := c.Post("/system-events/search", map[string]any{
					"q":              query,
					"from_date_time": "",
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'addigy-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
