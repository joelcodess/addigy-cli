...
Exit codes & warnings:
  ...
  addigy-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  addigy-cli sync --full

  # Incremental sync: only records from the last 7 days
  addigy-cli sync --since 7d

  # Parallel sync with 8 workers
  addigy-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  addigy-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/addigy-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "compliance_rules":
		return db.UpsertComplianceRules(data)
	case "ddm_updates":
		return db.UpsertDdmUpdates(data)
	case "device":
		return db.UpsertDevice(data)
	case "o_devices":
		return db.UpsertODevices(data)
	case "homescreen":
		return db.UpsertHomescreen(data)
	case "identity":
		return db.UpsertIdentity(data)
	case "integrations":
		return db.UpsertIntegrations(data)
	case "o_mdm":
		return db.UpsertOMdm(data)
	case "o_monitoring":
		return db.UpsertOMonitoring(data)
	case "policies":
		return db.UpsertPolicies(data)
	case "policy":
		return db.UpsertPolicy(data)
	case "o_prebuilt_apps":
		return db.UpsertOPrebuiltApps(data)
	case "reports":
		return db.UpsertReports(data)
	case "scripts":
		return db.UpsertScripts(data)
	case "templates":
		return db.UpsertTemplates(data)
	case "o_users":
		return db.UpsertOUsers(data)
	case "variables":
...
func defaultSyncResources() []string {
	return []string{
		...
		"feature-betas",
		"mdm",
		"mdm-configurations-definitions",
		"mdm-configurations-profiles",
		"monitoring",
		"oa",
		"oa-benchmarks-pre-built",
		"oa-compliance-rules-pre-built",
		"oa-integrations-azure-ca-tenants-metadata",
		"static-fields",
		"static-fields-value",
		"system-updates",
	}
	// Omitted from the default set because their endpoints require a path/query
	// param the bulk syncer cannot supply, so they always return HTTP 400 in a
	// full sync: "mdm-configurations-policy-profiles" (needs policy_id) and
	// "system-updates-available" (needs device_udid).
}
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		...
		"device-script-assignments":          "/device-script-assignments",
		"facts":                              "/facts/custom",
		"feature-betas":                      "/feature-betas",
		"mdm":                                "/mdm/profiles",
		"mdm-configurations-definitions":     "/mdm/configurations/definitions",
		"mdm-configurations-policy-profiles": "/mdm/configurations/policy/profiles",
		"mdm-configurations-profiles":        "/mdm/configurations/profiles",
		"monitoring":                         "/monitoring/received-alerts",
		"oa":                                 "/oa/reports/available",
		"oa-benchmarks-pre-built":            "/oa/benchmarks/pre-built",
		"oa-compliance-rules-pre-built":      "/oa/compliance-rules/pre-built",
		"oa-integrations-azure-ca-tenants-metadata": "/oa/integrations/azure-ca/tenants/metadata",
		"static-fields":            "/static-fields",
		"static-fields-value":      "/static-fields/value",
		"system-updates":           "/system-updates/latest",
		"system-updates-available": "/system-updates/available",
	}
...
var resourceIDFieldOverrides = map[string]string{
	...
	// MDM profile payloads key on payload_group_id (no plain "id"); without
	// this the generic fallback misses and every item is dropped at upsert.
