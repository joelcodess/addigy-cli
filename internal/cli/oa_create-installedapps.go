...
func newOaCreateInstalledappsCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:         "create-installedapps",
		Short:       "Query installed apps from a device providing some agent IDs. Permission Required: View Devices.",
		Example:     " addigy-cli oa create-installedapps",
		Annotations: map[string]string{"pp:endpoint": "oa.create-installedapps", "pp:method": "POST", "pp:path": "/oa/installed-apps/mdm/query"},
	...
	cmd.Flags().IntVar(&bodySkip, "skip", 0, "Skip")
	cmd.Flags().StringVar(&bodySortDirection, "sort-direction", "asc", "Sort direction")
	cmd.Flags().StringVar(&bodySortField, "sort-field", "name", "Sort field")
