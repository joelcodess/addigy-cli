...
import (
	...
	mcptools "github.com/joelcodess/addigy-cli/internal/mcp"
	"github.com/mark3labs/mcp-go/server"
)

// Transport selection order: --transport flag, then ADDIGY_MCP_TRANSPORT env,
...
// .goreleaser.yaml). Defaults to a dev marker for non-release builds.
var version = "1.0.0"

func main() {
	s := server.NewMCPServer(
		"Addigy Documentation",
		version,
	...
	case "http":
		...
		fmt.Fprintf(os.Stderr, "addigy-mcp serving MCP over streamable HTTP at %s\n", *addr)
...
// defaultTransport reads ADDIGY_MCP_TRANSPORT env when set, otherwise falls back
...
func defaultTransport() string {
	if t := os.Getenv("ADDIGY_MCP_TRANSPORT"); t != "" {
		return t
	}
	return "stdio"
}
